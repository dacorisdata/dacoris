...
export default function ProjectDetailPage() {
  ...
  const loadProject = async () => {
    try {
      ...
      setFormData({
        ...
        researchObjectives: draft.researchObjectives ?? parseJsonList(p.research_objectives),
  ...
  const buildProjectPayload = () => ({
    ...
    research_objectives: (formData.researchObjectives || []).filter(o => o.objective?.trim() || o.outcome?.trim()),
  ...
  const addObjective = () => {
    setFormData(p => ({
      ...p,
      researchObjectives: [...(p.researchObjectives || []), { objective: '', outcome: '' }],
    }));
  };
  ...
  const handleSaveDraft = async () => {
    try {
      ...
      await axios.patch(`${API}/research/projects/${id}`, buildProjectPayload(), { headers: authHeaders() });
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mb: 2 }}>
          {formData.researchObjectives.map((obj, index) => (
            <Box key={index} sx={{ display: 'flex', gap: 1.5, alignItems: 'flex-start' }}>
              <Box sx={{
                width: 28, height: 28, borderRadius: '50%', flexShrink: 0, mt: 0.5,
              ...
              <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                <TextField size="small" fullWidth label={`Objective ${index + 1}`}
                  value={obj.objective || ''} placeholder={`Objective ${index + 1}`}
                  onChange={e => updateObjective(index, 'objective', e.target.value)} sx={inp} />
                <TextField size="small" fullWidth label="Expected Outcome"
                  value={obj.outcome || ''} placeholder="Expected outcome of this objective"
                  onChange={e => updateObjective(index, 'outcome', e.target.value)} sx={inp} />
              </Box>
              <IconButton size="small" onClick={() => removeObjective(index)} sx={{ mt: 0.5, color: '#ef4444' }}>
